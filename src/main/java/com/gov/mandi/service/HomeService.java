...
public class HomeService {
	...
	public List<VegPrice> getVegetablePrice(Map<String,String> inMap)
	{
		...
		ResponseEntity<String> response
		  = restTemplate.getForEntity(mandipApi+ "&filters[state]="+inMap.get("state"), String.class);
			...
			root = mapper.readTree(response.getBody());
			 ...
			 if(count != null)
			 {
				 ...
				 for (int i=0;i< loopCount;i++)
				 {
					...
					response = restTemplate.getForEntity(mandipApi+ "&filters[state]="+inMap.get("state"), String.class);
